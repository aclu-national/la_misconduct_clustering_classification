...
# Turning the classifications into a list of classification for multi-label
...
for i, label in enumerate(label_columns):
    ...
    # Definign test matrics
    test_precision = precision_score(test_labels[label], test_preds[:, i], average='binary')
    test_recall = recall_score(test_labels[label], test_preds[:, i], average='binary')
    test_accuracy = accuracy_score(test_labels[label], test_preds[:, i])
    test_confusion = confusion_matrix(test_labels[label], test_preds[:, i])
    
    def extract_metrics(conf_matrix):
        if conf_matrix.shape == (2, 2): 
            tn, fp, fn, tp = conf_matrix.ravel()
        else:
          # Dealing with when there are not both 0 and 1 
            tn = fp = fn = tp = 0
...
predict("Weapon Misuse", pipeline)
